...
如果只需要刷新500网或澳客数据，可以使用独立脚本：

```bash
python run_collectors.py
```

脚本会把最新内容写到`site/data/jczq.json`和`site/data/history_okooo.json`，并自动保留旧数据以免意外。

> **注意**：2026 年初升级后曾出现爬虫输出乱码的问题，名称如
> `ΑΡ≥§`（希腊字母）等无法识别。原因在于 `requests` 的编码检测
> 返回了 `mac_greek`，造成内容被错误解码。最新版本会**强制使用
> gbk/gb2312** 并提供 `decode_response` 工具，重新运行即可恢复正确
> 中文数据。

## 🧪 运行测试

本项目包含简单的单元/集成测试，用于验证爬虫和管道是否能正常工作。安装依赖后运行：

```bash
pip install -r requirements.txt   # 包含 pytest
python -m pytest tests/test_collectors.py tests/test_pipeline.py -q
```

上述测试只有少量网络请求，不会影响500网或澳客站点。它们也可用作二次开发时的回归检查。
...
- 🐛 发现bug? [提交Issue](https://github.com/Bosun4/Foot/issues)
- 💡 有建议? [讨论](https://github.com/Bosun4/Foot/discussions)
